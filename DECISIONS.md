...
- **Host:** Render (Dockerized) behind Cloudflare, live at <https://options.hector-garza.com>. No API keys.
...
- **Health-check robustness (deploy):** gunicorn runs with `--preload` so the WSGI app — and its heavy transitive imports (plotly/pandas/scipy/yfinance, pulled in via `lab.views`) — load in the arbiter *before* the worker accepts traffic. Django resolves the URLconf lazily, so without preload the **first** request (which on Render is the `/healthz` probe) pays the full import cost; on a constrained instance that can starve the health check and time out the deploy. With preload, first-hit `/healthz` measured ~0.5s. **REJECTED** lazy first-request loading for the probe path. Note: the actual June-7 deploy timeout was Render's `free→starter` plan transition, not import latency (free/starter share compute and the app served 200 under prod settings) — this is hardening against a latent risk surfaced by that investigation, not the transition's cause.
